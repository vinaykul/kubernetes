...
func doPodResizeSchedulerTests() {
	...
	ginkgo.It("pod-resize-scheduler-testcase1", func() {
		...
		nodeAllocatableCPUValue := nodeAllocatableCPU.MilliValue()
		nodeAllocatableCPUStr := nodeAllocatableCPU.String()
		framework.Logf("Node allocatable CPU is %s", nodeAllocatableCPUStr)
		testPodRequest := nodeAllocatableCPUValue / 2
		testPodRequestStr := strconv.FormatInt(testPodRequest, 10) + "m"
		framework.Logf("Test Pod Request CPU is %s", testPodRequestStr)
		testPodRequestAfterResize := testPodRequest / 2
		testPodRequestAfterResizeStr := strconv.FormatInt(testPodRequestAfterResize, 10) + "m"
		framework.Logf("Test Pod CPU Request After Resize is %s", testPodRequestAfterResizeStr)
		testPodRequestToMakeSpace := testPodRequestAfterResize / 2
		testPodRequestToMakeSpaceStr := strconv.FormatInt(testPodRequestToMakeSpace, 10) + "m"
		framework.Logf("Test Pod CPU Request To Make Space is %s", testPodRequestToMakeSpaceStr)

		ginkgo.By("Making testPod1 that fits the node, testPod2 that cannot fit!")
		c1 := []TestContainerInfo{
			{
				Name:      "c1",
				Resources: &ContainerResources{CPUReq: testPodRequestStr, CPULim: testPodRequestStr},
			},
		}
		...
		c2 := []TestContainerInfo{
			{
				Name:      "c2",
				Resources: &ContainerResources{CPUReq: nodeAllocatableCPUStr, CPULim: nodeAllocatableCPUStr},
			},
		}

		patchC2ToFit := fmt.Sprintf(`{
			"spec":{
					"containers":[
						{
							"name":"c2",
			...
			}`, testPodRequestAfterResizeStr, testPodRequestAfterResizeStr)
		...
		testPod2, pErr := f.ClientSet.CoreV1().Pods(testPod2.Namespace).Patch(context.TODO(),
			testPod2.Name, types.StrategicMergePatchType, []byte(patchC2ToFit), metav1.PatchOptions{})
		framework.ExpectNoError(pErr, "failed to patch pod for resize")
		...
		patchC1ToMakeSpace := fmt.Sprintf(`{
			...
			}`, testPodRequestToMakeSpaceStr, testPodRequestToMakeSpaceStr)
