...
function execute-cmd-on-master-with-retries() {
  ...
  while true; do
    ...
    set +e
    ssh -o "StrictHostKeyChecking no" -t ${KUBE_SSH_USER}@${KUBEMARK_MASTER_IP} "$1"
    sshSts=$?
    set -e
    if [ $sshSts -ne 0 ]; then
...
function copy-files() {
  ...
  unset "array[${#array[@]}-1]"
  ssh -o "StrictHostKeyChecking no" -t ${KUBE_SSH_USER}@${KUBEMARK_MASTER_IP} "sudo mkdir -p /tmp/foo1234 && sudo chmod 777 /tmp/foo1234"
  for fl in "${array[@]}"; do
    scp -o "StrictHostKeyChecking no" -rp $fl ${KUBE_SSH_USER}@${KUBEMARK_MASTER_IP}:/tmp/foo1234
    fn=`basename $fl`
    ssh -o "StrictHostKeyChecking no" -t ${KUBE_SSH_USER}@${KUBEMARK_MASTER_IP} "sudo mv /tmp/foo1234/$fn $dest"
  done
  ssh -o "StrictHostKeyChecking no" -t ${KUBE_SSH_USER}@${KUBEMARK_MASTER_IP} "sudo rm -rf /tmp/foo1234"
}
