...
function enable-root-ssh() {
  sudo sed -i 's/.*sleep 10;exit 142\" //' /root/.ssh/authorized_keys
}
...
function unpack-kubernetes() {
  ...
  if [[ "${KUBERNETES_MASTER}" == "true" ]]; then
    ...
    for img in "${img_bins[@]}"; do
      ...
      sudo docker image tag k8s.gcr.io/${img}-amd64:${KUBE_VER} k8s.gcr.io/${img}:${KUBE_VER}
    done
  fi
  echo "Loading docker image for kube-proxy"
  sudo docker load -i ./kubernetes/server/bin/kube-proxy.tar
  sudo docker image tag k8s.gcr.io/kube-proxy-amd64:${KUBE_VER} k8s.gcr.io/kube-proxy:${KUBE_VER}
  popd
...
function setup-user-account() {
  set +e
  sudo getent passwd ${KUBE_SSH_USER} > /dev/null 2&>1
  USER_FOUND=$?
  set -e
  if [ $USER_FOUND -ne 0 ]
  then
    ...
    sudo cp -i /root/.ssh/authorized_keys /home/${KUBE_SSH_USER}/.ssh
    sudo echo ${KUBE_SSH_USER_CERT} >> /home/${KUBE_SSH_USER}/.ssh/authorized_keys
    sudo chown -R ${KUBE_SSH_USER}:${KUBE_SSH_USER} /home/${KUBE_SSH_USER}/.ssh
    echo "${KUBE_SSH_USER} ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers
  fi
}
...
function setup-kubernetes-master() {
  ...
  sed -i "s/port=0/port=10252/" /etc/kubernetes/manifests/kube-controller-manager.yaml
  sed -i "/HTTPS/d" /etc/kubernetes/manifests/kube-controller-manager.yaml
  sed -i "s/10257/10252/" /etc/kubernetes/manifests/kube-controller-manager.yaml
  sed -i "s/port=0/port=10251/" /etc/kubernetes/manifests/kube-scheduler.yaml
  sed -i "/HTTPS/d" /etc/kubernetes/manifests/kube-scheduler.yaml
  sed -i "s/10259/10251/" /etc/kubernetes/manifests/kube-scheduler.yaml
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "${KUBE_SSH_USER}" != "root" ] && [ "${KUBE_SSH_USER}" != "ubuntu" ]; then
      sudo mkdir -p /home/${KUBE_SSH_USER}/.kube
      sudo cp -i /etc/kubernetes/admin.conf /home/${KUBE_SSH_USER}/.kube/config
      sudo chown -R ${KUBE_SSH_USER}:${KUBE_SSH_USER} /home/${KUBE_SSH_USER}/.kube
    fi
